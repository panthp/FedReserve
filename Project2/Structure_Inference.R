...
ggplot(d,aes(x = value)) + 
  ...
  geom_histogram() #Plot histogram of data

#NonParanormal Transform (npn)
NonParaTrans = huge.npn(copy_Processed_data)#, npn.func = "skeptic")
NonParaTrans = as.data.frame(NonParaTrans)
d <- melt(NonParaTrans)
ggplot(d,aes(x = value)) + 
  facet_wrap(~variable,scales = "free_x") + 
  geom_histogram(binwidth = 0.50) #Confirm data is now Gaussian

####PC####
copy_Processed_data = NonParaTrans
copy_Processed_data = subset(copy_Processed_data, select = -c(DATE))
suffStat <- list(C = cor(copy_Processed_data), n = nrow(copy_Processed_data))
pc.data <- pc(suffStat, indepTest = gaussCItest, labels = names(copy_Processed_data), alpha = 0.005)
...
####Run Grow-Shrink Algorithm####
res = gs(copy_Processed_data)
plot(res)

###Run Max-Min Hill Climbing algorithm###
...
copy_Processed_data = Processed_Data
copy_Processed_data = subset(copy_Processed_data, select = -c(DATE))
data_var <- var(copy_Processed_data) #Compute covariance of data for Glasso
